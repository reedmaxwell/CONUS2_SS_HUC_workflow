...
real*8 Clocx, Clocy, Clocz, Z, maxz, Xdem,xdem1,xdem2, Ydem, Zdem
...
! prior particle locations, used to check for stagnation
...
if (StreamName /= '') then
 ...
 call pfb_read(streams,fname,nx,ny,nztemp)
end if ! streams
...
k = nz
 ...
 P(ii,15) = part_tstart + 0.0 !setting insert time to the start time
  ...
  P(ii,3) = Z  -dz(k-1)  !(DEM(i,j)-dz(k))  !-maxZ)+Z !-dz(k)*ran1(ir)
  !print*, P(ii,3), DEM(i,j), i,j
  PInLoc(ii,3) = P(ii,3)
  P(ii,14)=P(ii,3) ! Saving the initial location
        ...
        ! and the water contained in that cell
        P(ii,6) = dx*dy*dz(k)*(Porosity(i,j,k)  &
                 *Saturation(i,j,k))*denh2o*(1.0d0/float(abs(np_ic)))
        P(ii,7) = 1.0d0
        P(ii,8) = 1.0d0
        ! set up intial concentrations
        C(1,i,j,k) = C(1,i,j,k) + P(ii,8)*P(ii,6) /  &
  ...
  write(11,*) ' **Warning IC input but no particles left'
  write(11,*) ' **Exiting code gracefully writing restart '
  goto 9090
...
write(11,*) 'number of river particles:',np_active
end if !! river IC
...
! open / create a particle binary file that writes all particle attributes each timestep
open(314,file=trim(runname)//'_streamline_trace.bin', FORM='unformatted',  &
...
!$OMP PARALLEL PRIVATE(Ploc, k, l, ik, Clocx, Clocy, Clocz, Vpx, Z, z1, z2, z3, xold, yold, zold)   &
!$OMP& PRIVATE(Vpy, Vpz, particledt, delta_time,local_flux, et_flux)  &
!$OMP& PRIVATE(water_vol, Zr, itime_loc, advdt, DR_Temp, ir, xdem1, xdem2, ydem, zdem) &
!$OMP& SHARED(BoundFlux, EvapTrans, Vx, Vy, Vz, P, Saturation, Porosity, dx, dy, dz, denh2o, Ind, dem,streams) &
...
!$OMP DO
        ...
        if(P(ii,8) == 1.0) then
                ...
                (streams(Ploc(1)+1,Ploc(2)+1)  == 1.0).and.  &
...
! !                !flag particle as inactive
                ...
                goto 999
                        ...
                        ! check to see if we are on the boundary w/ a zero velocity
                        if ((Vx(Ploc(1)+1,Ploc(2)+1,Ploc(3)+1)==0.0).or.(Vx(Ploc(1)+2,Ploc(2)+1,Ploc(3)+1)==0.0)) then
                                        P(ii,8) = 0.0d0
                                        !flag as exiting via stagnation
                                        P(ii,10) = 4.0d0
                                        goto 999
                        end if
                        if ((Vy(Ploc(1)+1,Ploc(2)+1,Ploc(3)+1)==0.0).or.(Vy(Ploc(1)+1,Ploc(2)+2,Ploc(3)+1)==0.0)) then
                                        P(ii,8) = 0.0d0
                                        !flag as exiting via stagnation
                                        P(ii,10) = 4.0d0
                                        goto 999
                        end if
                        !if ((Vz(Ploc(1)+1,Ploc(2)+1,Ploc(3)+1)==0.0).or.(Vy(Ploc(1)+1,Ploc(2)+1,Ploc(3)+2)==0.0)) then
                        !                P(ii,8) = 0.0d0
                        !                !flag as exiting via stagnation
                        !                P(ii,10) = 4.0d0
                        !                goto 999
...
!!
                        ...
                        !! for this case only we pull any particles that reach the boundary
                        P(ii,8) = 0.0d0
                        goto 999
                        ...
                        ! double check for stagnating particles
                        
                                if ((P(ii,1) == Xold).or.(P(ii,2) == Yold).or.(P(ii,3) == Zold)) then
                                        P(ii,8) = 0.0d0
                                        !flag as exiting via stagnation
                                        P(ii,10) = 4.0d0
                                        goto 999
                                end if
                                if ((Vpx == 0.0).or.(Vpy == 0.0).or.(Vpz == 0.0)) then
                                        P(ii,8) = 0.0d0
                                        !flag as exiting via stagnation
                                        P(ii,10) = 4.0d0
                                        goto 999
                        ...
                        Clocx = (P(ii,1) - float(Ploc(1))*dx)  / dx
                        Clocy = (P(ii,2) - float(Ploc(2))*dy)  / dy
                        ! first check to see if we are in the upper or lower half of the cell in Y
                        if (Clocy <= 0.5d0) then  !lower half in Y Ploc(2)
                        ! then check to see if we are in the left or right half of the cell in X
                                if (Clocx <= 0.5d0) then
                                Xdem1 = DEM(Ploc(1),Ploc(2))*((1.0-Clocx)-0.5) +(Clocx+0.5)*DEM(Ploc(1)+1,Ploc(2))
                                Xdem2 = DEM(Ploc(1),Ploc(2)+1)*((1.0-Clocx)-0.5) +(Clocx+0.5)*DEM(Ploc(1)+1,Ploc(2)+1)
                                else
                                Xdem1 = DEM(Ploc(1)+1,Ploc(2))*((1.0-Clocx)+0.5) + (Clocx-0.5)*DEM(Ploc(1)+2,Ploc(2)) 
                                Xdem2 = DEM(Ploc(1)+1,Ploc(2)+1)*((1.0-Clocx)+0.5) + (Clocx-0.5)*DEM(Ploc(1)+2,Ploc(2)+1)  
                                end if
                                Zdem = Xdem1*((1.0-Clocy)-0.5) +(Clocy+0.5)*Xdem2
                        else   ! upper half in Y Ploc(2)
                                if (Clocx <= 0.5d0) then
                                Xdem1 = DEM(Ploc(1),Ploc(2)+1)*((1.0-Clocx)-0.5) +(Clocx+0.5)*DEM(Ploc(1)+1,Ploc(2)+1)
                                Xdem2 = DEM(Ploc(1),Ploc(2)+2)*((1.0-Clocx)-0.5) +(Clocx+0.5)*DEM(Ploc(1)+1,Ploc(2)+2)
                                else
                                Xdem1 = DEM(Ploc(1)+1,Ploc(2)+1)*((1.0-Clocx)+0.5) + (Clocx-0.5)*DEM(Ploc(1)+2,Ploc(2)+1) 
                                Xdem2 = DEM(Ploc(1)+1,Ploc(2)+2)*((1.0-Clocx)+0.5) + (Clocx-0.5)*DEM(Ploc(1)+2,Ploc(2)+2)  
                                end if
                                Zdem = Xdem1*(1.0-Clocy+0.5) + (Clocy-0.5)*Xdem2
                        end if
                         

                        if (Zdem < 0.0) print*, 'XDEM', Clocx, Ploc(1), Ploc(2), P(ii,1), P(ii,2), Xdem
                        if (Xdem1 < 0.0) print*, 'YDEM', Clocy, Ploc(1), Ploc(2), P(ii,1), P(ii,2), Ydem
                         if (Xdem2 < 0.0) print*, 'YDEM', Clocy, Ploc(1), Ploc(2), P(ii,1), P(ii,2), Ydem
                        ! write out time to 3D ASCII file that paraview can read
                        if (P(ii,8) == 1.0d0) write(214,61) P(ii,1), P(ii,2), (Zdem -Zmax) +P(ii,3), P(ii,4)  !P(ii,16)  !P(ii,4), P(ii,16)
                        ! write out all particles to binary file
                                ! P(np,1) = X coordinate [L]
                                ! P(np,2) = Y coordinate [L]
                                ! P(np,3) = Z coordinate [L]
                                ! P(np,4) = Particle residence time [T]
                                ...
                                ! P(np,16) = Length of flow path [L]
                                ! P(np,17) = Length of saturated flow path [L]
                        if (P(ii,8) == 1.0d0) write(314)  P(ii,1), P(ii,2), P(ii,3), &
